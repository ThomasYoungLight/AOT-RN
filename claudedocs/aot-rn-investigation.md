...
### Experiment 19 (2026-08-02): concurrent lanes, priorities, and Suspense under a ConcurrentRoot

The last big reconciler gap is closed: the typed port now implements React 18.3.1's **concurrent architecture** — the full lane model, the scheduler contract, interruptible rendering, transitions, and Suspense under a concurrent root — verified equivalent to the real reconciler on a **third axis** beyond host mutations and effect order: a **scheduler-trace checksum**.

**The determinism trick.** Concurrent React is scheduled by the `scheduler` package (wall-clock deadlines, MessageChannel). For byte-exact verification it is replaced (esbuild `--alias`) by `det-scheduler.cjs`: a **frozen clock** that only advances when the driver says so, an **explicit pump** (`flushOne(budget)`/`flushAll()`, ordering = (expirationTime, taskId) — the real heap order under a frozen clock), and a **yield budget** (`shouldYield()` returns true after N calls, so `workLoopConcurrent` yields at driver-chosen work-unit counts). Every schedule/cancel/run/continuation/yield event feeds `__schedTrace.sum`. The typed port implements the same scheduler in the typed subset and mixes the same trace: **equal sums mean the two reconcilers scheduled, cancelled, yielded, and resumed at identical work-unit boundaries** — not just that they committed the same trees.

**Ported (from the 0.29.2 source, both root modes co-existing):**
- **Lane model** — full 18.3.1 bitfields (Sync/InputContinuous/Default/16 transition/5 retry/Idle/Offscreen lanes), `getNextLanes` with wip-interruption rules and **transition entanglement**, starvation/expiration bookkeeping (`markStarvedLanesAsExpired`, per-lane `eventTimes`/`expirationTimes` maps), `claimNextTransitionLane` cycling.
- **The work loop, replaced wholesale** — the port's ad-hoc legacy batching (`isBatching`/`pendingRoot`) is gone; in its place React's real `executionContext` + `syncQueue`/`flushSyncCallbacks` + `ensureRootIsScheduled` (callback reuse by priority, cancellation) + `performConcurrentWorkOnRoot` (passive-flush preamble, `shouldTimeSlice`, exit statuses, **continuations** back to the scheduler) + `commitRoot(Impl)` with lane bookkeeping (`markRootFinished`, remaining-lane rescheduling, discrete-passive sync flush). Legacy mode now rides the same machinery (`scheduleLegacySyncCallback` + `flushSyncCallbacksOnlyInLegacyMode`) — and the Experiment 15/18 gates confirmed byte-identical legacy behavior after the swap.
- **Lane-aware hooks** — `fiber.mode`, `requestUpdateLane` (transition claim, update-priority, event-priority), the 18.3 **interleaved concurrent update queue** (`queue.interleaved` + `finishQueueingConcurrentUpdates` at `prepareFreshStack`), and the full **`updateReducer` rebase algorithm**: skipped updates become the new base queue (clones, `NoLane` for already-committed ones), skipped lanes re-marked on fiber and root — the merged queue written to the CURRENT hook so interrupted renders survive restarts.
- **`useTransition` / `startTransition` / `useDeferredValue`** — pending-state flips at continuous priority, per-event transition lanes, deferred values spawning transition-lane catch-up renders (`hook.baseState` reuse as the staleness bit).
- **Concurrent Suspense** — `markSuspenseBoundaryShouldCapture` (concurrent: `ShouldCapture` + boundary lanes; legacy branches preserved under mode gates), **ping listeners** (`root.pingCache`, `pingSuspendedRoot` with mid-render restart rules), **retry lanes** (`claimNextRetryLane` via `resolveRetryWakeable`), `renderDidSuspend`/`renderDidSuspendDelayIfPossible` with the suspense-context stack (`InvisibleParentSuspenseContext`), fallback **throttle** (`markCommitTimeOfFallback` + 500 ms window) and **JND delay** — where React arms a host timeout, the harness relies on the twin's host config returning `-1` (timeouts never fire), so *waiting for the data ping* is the verified behavior on both sides; `attemptEarlyBailoutIfNoScheduledUpdate` (bailed-over boundaries skip the hidden primary, offscreen re-enters the deferral decision); **Offscreen render-lane stack** (`pushRenderLanes`/`popRenderLanes`, hidden-tree `baseLanes`, OffscreenLane deferral) and the **hidden-tree effect disconnect**: `disappearLayoutEffects` on hide (all layout destroys + ref detach), `reappearLayoutEffects` on unhide, `offscreenSubtreeIsHidden/WasHidden` gating layout mounts, ref attach/detach, and deletion-time destroys.

**Workload** (`concurrent-app.inc.js`, shared verbatim): 60–70 memoized rows + status/preview panes on a ConcurrentRoot, driven through automatic batching, sliced default-lane renders **interrupted mid-flight by discrete updates** (sync preemption → skipped-update rebase → restart), marks + structural placements/deletions inside transitions, two-transitions-one-lane entanglement, `useDeferredValue` catch-up, passive-effect setState cascades — plus a deterministic Suspense lifecycle over a lazy panel with synchronous thenables: default-lane mount-suspend (fallback commits, retry suspends on data inside the throttle window), two update-suspends that **hide committed content** (JND wait → fallback commit → disappear/reappear layout effects), a **suspended transition that never commits a fallback** (waits for the ping, `isPending` held), and subtree unmount.

**Result: all three axes identical, twin vs typed port, in BOTH host modes** — mutation: host `1711752146` / `fx 186024253` / sched `2520337350`; **persistence (Fabric-shaped, what a new-arch ring 0 runs): host `2588328100`** with the same fx and sched sums (the render/schedule behavior is host-independent — a strong internal consistency check), hiddenClones=4, 8,706 clones. Identical event counts throughout (2,732 schedules / 270 cancels / 2,570 task runs / **105 mid-render yields with continuations** / 56,647 work-unit yield checks). Both legacy gates unchanged (`424919205/1208032539`, `1263784292/1208032539`). The orchestrator gained both concurrent pairs as standing gates (step 4.5).

**Perf (Mac, 800 interactions):** typed native **0.160 ms/interaction** vs interpreted real React **0.363** → **2.27×** mutation; **0.180 vs 0.408 → 2.26×** persistent — the largest reconciler speedups measured so far. Concurrent mode does strictly more per-unit bookkeeping (lane math, scheduler queues, restart re-renders), and that bookkeeping is exactly what the typed compiler accelerates.

**Device round** (rebuilt ring-0 units containing the concurrent-capable port, via `orchestrate.py --ios --install-android --install-ios`): both phones run the port as ring 0 with identical checksums (`424919205/1208032539`) — S23 Ultra 0.755 ms/interaction, iPhone 15 Pro Max 0.448 (vs 0.729/0.443 in Experiment 18: the unified lanes/scheduler work loop costs ≈0–3% on the legacy path). SHUnit eval unchanged (core 0.08–0.27 ms, ring1 1.8 ms); HybridShop TTI unchanged (iPhone 109 ms).

**Divergences the harness caught:**
1. Through-`any` writes to `HookUpdate.next` in circular-list splices (interleaved-queue transfer, base-queue merge) — the Experiment-18 runtime-check rule again; fixed with typed locals.
2. The port initially treated the fallback-throttle/JND windows as driver errors (explicit throw); the twin showed React **arms a host timeout and waits** — with never-firing host timers, "wait for the ping" is the correct, reproducible semantics (`hostTimeouts: 2` counted on the twin).

Still out of scope: class components, hydration, SuspenseList, `useSyncExternalStore`, render-phase hook updates (explicit fail-fast throw). Scope for RN: the new architecture runs React on a ConcurrentRoot — this experiment makes the typed ring-0 port viable for it, transitions and all.

Artifacts: `det-scheduler.cjs`, `concurrent-app.inc.js`, `main-real-concurrent-body.js` + `build-real-concurrent.sh` (twin), `typed-main-concurrent.ts` + `build-typed-concurrent.sh` (port), ~1,400 changed lines in `typed-port-core.ts`, concurrent gate in `orchestrate.py`.

### Experiment 20 (2026-08-02): useSyncExternalStore, the next Fabric clone contract, and a responder system

Three production-hardening items in one pass.

**20a — `useSyncExternalStore`** (the one remaining hook with real-world reach: Redux/Zustand adapters). Ported from 0.29.2: the store instance on `hook.queue`, the subscribe effect (`deps=[subscribe]`), the always-armed passive `updateStoreInstance` re-check (mutations between render and commit force a sync re-render), and the **concurrent pre-commit consistency check**: `pushStoreConsistencyCheck` (`StoreConsistency` flag, checks on the FC update queue) plus the `isRenderConsistentWithExternalStores` tree walk wired into `performConcurrentWorkOnRoot` — an inconsistent concurrent render is re-rendered synchronously to block further tearing. Workload: a non-memoized store badge; notifying updates on the common path, and a two-step tear scenario (a silent mutation before the transition render arms the check via `snapshotChanged`, a second one during a mid-render yield trips it — verified firing twice via a temporary counter). A finding worth recording: **a silent mutation against a render whose snapshot hadn't changed is missed by design** — React only arms consistency checks when the store binding changed during that render; the twin agrees byte-for-byte. All gates exact: concurrent mutation `1700871869 / fx -1912530176 / sched 3771290918`, concurrent persistent `3626803472` (same fx/sched), legacy pair unchanged.

**20b — `passChildrenWhenCloningPersistedNodes`** (Fabric contract tracking). RN's C++ marks the child-set bindings "TODO: remove"; the next contract passes children as **plain JS arrays** at clone/completeRoot time (`cloneNodeWithNewChildren(node, children)`, `completeRoot(tag, childArray)` — `shadowNodeListFromValue` already accepts arrays). The npm reconciler predates the flag, so op-stream twin verification is impossible **by design** — the equivalence axis is a new **committed-tree checksum** (types/text/props/structure hashed at every root completion, `tree=` on the stats line). Port: a build-flipped const, a `collectAllChildrenInto` walk (same traversal as the append walks, including the Offscreen hidden-clone branch), and contract branches in `completeWork`/`updateHostContainer`/the HostRoot commit; recording host and fabric host both gained the two pass-children ops. **Verified: committed trees identical across twin(old) == typed(old) == typed(pass-children)** on both persistent workloads — legacy `tree=2981923365`, concurrent `tree=4274245300` — with `childSets=0` under the new contract (the deprecated bindings never called) and hidden clones flowing through the array path. The live-Fabric ring-0 unit now ships the new contract permanently (the twin inherently drives the old one; both produce the same trees).

**20c — gesture responder system for the takeover path.** The Exp-13 demo's walk-to-nearest-`onPress` is replaced by a scoped port of `ResponderEventPlugin` over the live fiber tree (`responder.inc.js`, shared verbatim by twin and typed unit): capture/bubble negotiation (`onStartShouldSetResponder(Capture)`, `onMoveShouldSetResponder(Capture)`), responder transfer (`onResponderTerminationRequest` defaulting to allow, `onResponderTerminate`/`onResponderReject`), and `onResponderStart/Move/End/Release` delivery, each native event dispatched inside one sync batch (RN's discrete batching). Gesture origin tracking exposes `gestureDX/DY` to handlers. The demo app became Pressability-lite: rows highlight on grant, select on release, cancel on terminate; the list container simulates scroll by stealing the responder via `onMoveShouldSetResponderCapture` once a gesture moves past a 24 dp slop — a visible `scrollSteals` counter. Handlers must be referentially stable (`useCallback`) because instanceHandles persist across clones — the same constraint ReactFabric has. Multi-touch responder hand-off is out of scope (single-gesture semantics).

**A semantics bug only the device caught:** the first cut negotiated among views *deeper* than the current responder; real RN (`accumulateTwoPhaseDispatchesSkipTarget`) negotiates over the **responder's ancestors** — which is exactly how a scroll container steals an in-flight press. With the inverted direction the on-device drag selected the row instead of stealing; one line of `ResponderEventPlugin` reading later, the steal worked.
...
**iOS (Experiment 22b), same method, 83 screens** (the iOS example list is larger than Android's 76): navigation via `devicectl process launch --payload-url rntester://example/<key>`; **hybrid (523 native modules) and control (`__nativeModules: ABSENT`) are identical** — 83/83 screens visited on both, app alive at the end of both runs, and the error sets match exactly (the same two conditionally-registered Observer examples, nothing else). No crashes, no JS exceptions, on either build.

iOS forced three harness/product problems into the open, none of which any benchmark or gate would have caught:

1. **JS errors were invisible.** iOS release compiles out `RCTLog`, so a red-box error never reaches a device console — a "clean" iOS sweep would have been meaningless. Fixed with a test-harness **error bridge** in the demo module (`ErrorUtils.setGlobalHandler` + a `console.error` wrapper routed through the `__hybridLog` glog host function with a greppable `[HybridErr]` prefix). Verified by requesting a bogus screen key and watching it surface. The same bridge now gives Android a uniform signal.
2. **iOS could not build without units at all — the fail-safe was broken at link time.** `__attribute__((weak_import))` creates the weak reference, but ld64 still hard-errors on a static-only link where nothing *defines* the symbol. Android gets the graceful path for free (CMake `EXISTS` omits the object; weak ELF symbols resolve to 0). The iOS integration additionally needs `-Wl,-U,_sh_export_core -Wl,-U,_sh_export_ring1 -Wl,-U,_sh_export_fabriccore` in `OTHER_LDFLAGS` — now permanent, not just a control-build workaround. Without it, an integrator who builds before generating units gets a linker error instead of an interpreted app.
3. **An all-interpreted build had no log channel whatsoever**, because `__hybridLog` was installed *inside* the units-present branch. Diagnostics must not depend on the feature they diagnose: the bridge is now installed unconditionally in `ReactInstance.cpp`.

**Visual parity on iOS IS covered — on the physical device.** The first pass concluded it was impossible (no `devicectl` screenshot subcommand; libimobiledevice's `screenshotr` service returns "Invalid service" on iOS 26), but that only rules out *host-side* capture. RNTester links a **`ScreenshotManager` TurboModule** that renders the key window with `UIGraphicsImageRenderer`/`drawViewHierarchyInRect` and writes a PNG into the app container. So the app screenshots itself: the harness fires a second deep link `rntester://hybridshot/<key>` (RNTester ignores non-`example/` URLs with a harmless `console.warn`), a listener in the demo module captures and logs the path, and `devicectl device copy from --domain-type appDataContainer` pulls the file. Full-resolution 1290×2796 captures, no XCUITest target and no WebDriverAgent. A bonus: the in-app renderer captures only the app's key window, so there is no status-bar clock or home indicator to crop — iOS captures are *more* deterministic than Android's `screencap`.

**iOS result: 79/83 screens pixel-identical, 0 real divergences**, errors and liveness identical across variants. The 4 differing screens are all animated and each exceeds its cross-variant delta in same-binary noise.

**A measurement lesson worth generalizing:** a self-baseline built from two full sweeps *systematically under-reports periodic animation*, because a fixed-cadence sweep samples a spinner at correlated phases. `ActivityIndicatorExample` showed 802 cells cross-variant against **0** in the first hybrid self-baseline — which looked like a real divergence. Capturing that one screen six times back-to-back on a single build measured its true noise at **981 cells**. The two techniques are complementary and both are needed: repeat-capture catches steady-state animation (ActivityIndicator 981, Transform 22,249 cells), while sweep-to-sweep baselines catch *mount-time* animation that repeat-capture misses entirely (`AnExApp` measures 0 in back-to-back captures but ~44,000 cells across fresh navigations). The tool now has `sweep noise --screens … --repeats N`, and `--baseline` accepts a comma-separated union of sources, taking the worst case per screen.

**iOS soak (300 events, both variants): clean and identical.** iOS has no `adb monkey` and no supported touch injection outside XCUITest/WebDriverAgent, so `sweep soak --target device` stresses what the hybrid layer actually changes — randomized navigation, mount/unmount churn, native-module calls, and **cold start** — rather than gestures (Android's monkey already covered random touch). Hybrid and control both completed 300/300 events with 22/22 verified process restarts, stayed alive, and logged zero errors.

Two harness bugs surfaced here too, both of which would have produced wrong conclusions: a `devicectl launch --console` session *owns* the app process and kills it at teardown, so liveness read after teardown always looked like a crash (now sampled before); and `--terminate-existing` can be silently refused while a console session holds the process, so the first run claimed 22 cold relaunches while observing zero PID changes — the soak now verifies the PID after each relaunch and reports *verified* restarts.

**Not covered:** iOS 17+ removed the classic `screenshotr` lockdown service (libimobiledevice reports "Invalid service" on iOS 26) and `devicectl` has no screenshot subcommand, so device pixel capture is unavailable. The simulator path *is* implemented in the tool (`--target simulator`, using `simctl openurl` + `simctl io screenshot`, which would also let CI gate visual parity with no phone attached), but it needs hermes built for the simulator — this workspace has only `Pods/hermes-engine/build/iphoneos`, so that build chain (hermes → pods → app for the simulator) remains the open task for iOS pixel parity.

Caveat: this is one app on two devices (Galaxy S23 Ultra, iPhone 15 Pro Max). It is a *broad* app — the component and API surface is essentially RN's whole public catalog — but breadth of surface is not the same as breadth of hardware or of third-party native modules.
